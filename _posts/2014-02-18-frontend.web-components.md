...
В настоящее время HTML не предоставляет нативного механизма шаблонизации и для определения шаблонов на стороне браузера используют различные "приемы", позволяющие с разной степенью удобства эмулировать механизмы шаблонизации.  
...
Шаблоны имеют следующие свойства:
...
>        template = document.querySelector('#example-6 > .initial'),
>        root = host.createShadowRoot();

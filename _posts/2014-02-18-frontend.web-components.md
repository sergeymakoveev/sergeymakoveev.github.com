...
Для использования контента *host*-ноды в ее *ShadowRoot* задан *insertion point* (*курсор*) `<content />`.  
Курсор не меняет DOM-дерево *host*-ноды, а задает правила его отрисовки в *ShadowRoot*.
...
![](/assets/posts/2014-02-19/example.4.jpg)
{: .screenshot }

### Курсор `<content />`. Аттрибут `select`

Для *insertion point* `<content />` предусмотрен аттрибут *select*, значением которого являет список простых css-селекторов, разделенный запятыми.  
Аттрибут предоставляет возможность выбора части контента *host*-ноды для указания ее места в *ShadowRoot*.  
В предыдущих примерах уже использовался курсор `<content></content>`, что равнозначно `<content select=""></content>` или `<content select="*"></content>` - т.е. происходит выбор всего контента *host*-ноды.  
Следующий пример использует эту возможность.  
Научим элемент корректно обрабатывать предустановленные значения (не js-хардкодом, а гибко - в верстке).  

...
Исследуем возможности *Shadow DOM* на примере простого элемента `<button />`.  
...
Поэтому контент, распределенный в *Shadow DOM* точками вставки не виден селекторам как из дерева основного документа, так и из *ShadowRoot*-дерева.
...
В спецификации декларируется элемент "HTMLTemplateElement" `<template></template>`, который позволяет объявлять фрагменты документа в качестве шаблонов.  
...
(function(){
    var example = document.querySelector('#example-6'),
        ...
        tmpl_button = example.querySelector('> .button'),
        tmpl_time = example.querySelector('> .time'),
        root = host.createShadowRoot();
...
Стили, которые определены в shadow tree имеют силу только внутри этого дерева.

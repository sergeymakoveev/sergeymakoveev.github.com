...
{: .fragment .highlight-current-blue .no-border}
Почему не написано UI-фреймворка, который можно использовать с любым
...
## JS-фреймворки: выбор и миграция
...
Причем со стороны заказчика считается что:  
...
Таким образом, от пректа к проекту, происходит повторяющийся процесс разработки
...
*   {: .fragment .highlight-current-blue .no-border}
    вебкомпоненты работают со своим Shadow DOM, не оказывая влияния на virtual DOM фреймворка

*   {: .fragment .highlight-current-blue .no-border}
    протекающие внутри вебкомпонентов процессы изолированы от внешнего влияния и 
    сами не оказывают влияния на внешнюю среду фреймворка

*   {: .fragment .highlight-current-blue .no-border}
    взаимодействие с внешней средой фреймворка происходит через
    js-api вебкомпонента и его атрибуты
...
*   {: .fragment .highlight-current-blue .no-border}
    **использование Shadow DOM** в polymer-компонентах позволяет создать:  
    — локальное изолированное DOM-дерево пользовательского элемента  
    — shadow-дерево пользовательского элемента на основе DOM-template
...
**Полная совместимость по API со стеком стандартов Web Components позволяет
переключаться на использование браузерных реализаций вебкомпонентов при их наличии**
...
*   использует Bower для управления зависимостями  
    ( имеет плоское дерево зависимостей - этом упрощаются относительные пути в импортах )
